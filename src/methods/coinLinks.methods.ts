...
import { ICoinLinks } from "../types/coin.types";
...
export const addCoinLinksIfNotExists = async (coinLinksData: ICoinLinks): Promise<ICoinLinks[]> => {
  const query = `
        ...
        RETURNING *;
    `;
  return executeQuery<ICoinLinks>(
    query,
    [
...
export const addMultipleCoinLinksOrUpdate = async (coinLinksData: ICoinLinks[]): Promise<ICoinLinks[]> => {
  const query = `
        INSERT INTO coin_links (coin_id, twitter, telegram, discord, github, website, whitepaper, medium, linkedin, youtube, reddit, facebook, instagram, tiktok, forum, other_links, tags)
  ...
  const values = coinLinksData.reduce<any[]>((acc, coinLinks) => {
  ...
  return executeQuery<ICoinLinks>(query, values);
};

export const updateCoinLinks = async (coinId: string, coinLinksData: Partial<ICoinLinks>): Promise<ICoinLinks[]> => {
  const query = `
        UPDATE coin_links
        SET twitter = $1,
            telegram = $2,
...
export const getCoinLinksByCoinId = async (coinId: string): Promise<ICoinLinks[]> => {
  const query = `
        SELECT * FROM coin_links WHERE coin_id = $1;
    `;
  return executeQuery<ICoinLinks>(query, [coinId]);
};

export const getAllCoinLinks = async (): Promise<ICoinLinks[]> => {
  const query = `
        SELECT * FROM coin_links;
    `;
  return executeQuery<ICoinLinks>(query);
};
