...
const fetchCoinDetailsfromAddressAndAggregateData = async(address) => {
    ...
    for (const pair of apiRes.pairs) {
        ...
        // aggregate price
        if (address === pair.baseToken.address && pair.priceUsd != undefined && index <= 5){
            average_price_numenator = average_price_numenator.plus(Big(pair.priceUsd)) ;
        ...
        } else if (address === pair.quoteToken.address && pair.priceNative != undefined && index <= 5) {
            average_price_numenator  = average_price_numenator.plus(Big(pair.priceUsd / pair.priceNative)) ; 
            average_price_denominator += 1;
        ...
        if (pair.volume != undefined && index <= 5) {
            responseSkeletom.volume.h24 = responseSkeletom.volume.h24.plus(Big(pair.volume.h24));
            responseSkeletom.volume.h6 = responseSkeletom.volume.h6.plus(Big(pair.volume.h6));
            responseSkeletom.volume.h1 = responseSkeletom.volume.h1.plus(Big(pair.volume.h1));
            responseSkeletom.volume.m5 = responseSkeletom.volume.m5.plus(Big(pair.volume.m5));
        }

        // aggregate price change from all pools, 
        // can change to only top 5 pools in future if needed
        if (pair.priceChange != undefined && index <= 5) {
            responseSkeletom.priceChange.m5 = responseSkeletom.priceChange.m5.plus(Big(pair.priceChange.m5)); 
            responseSkeletom.priceChange.h1 = responseSkeletom.priceChange.h1.plus(Big(pair.priceChange.h1));
            responseSkeletom.priceChange.h6 = responseSkeletom.priceChange.h6.plus(Big(pair.priceChange.h6));
            responseSkeletom.priceChange.h24 = responseSkeletom.priceChange.h24.plus(Big(pair.priceChange.h24));
    ...
    if (average_price_denominator > 0) {
        responseSkeletom.price = average_price_numenator.div(Big(average_price_denominator));
    } 

    if (price_change_denominator > 0) {
        responseSkeletom.priceChange.m5 = responseSkeletom.priceChange.m5.div(Big(price_change_denominator));
        responseSkeletom.priceChange.h1 = responseSkeletom.priceChange.h1.div(Big(price_change_denominator));
        responseSkeletom.priceChange.h6 = responseSkeletom.priceChange.h6.div(Big(price_change_denominator));
        responseSkeletom.priceChange.h24 = responseSkeletom.priceChange.h24.div(Big(price_change_denominator));
    }
