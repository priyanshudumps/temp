...
async function runMigrations(direction, targetFile) {
  const pool = await db();
  ...
  try {
    ...
    let migrationFiles = fs.readdirSync(path.join(__dirname, 'sql'))
    ...
    for (const file of migrationFiles) {
      ...
      const { rows } = await client.query('SELECT 1 FROM migrations WHERE file_name = $1', [baseFileName]);
      ...
      if (direction === 'up') {
        await client.query('INSERT INTO migrations (file_name) VALUES ($1)', [baseFileName]);
      } else {
        await client.query('DELETE FROM migrations WHERE file_name = $1', [baseFileName]);
      }
...
//   console.error('Please specify the migration direction: up or down.');
//   process.exit(1);
// }
